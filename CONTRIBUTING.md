...
!!! note "提醒：PR 提交后继续推送 commit 会自动同步"

    提交 PR 之后，如果您继续在 PR 所属的同一分支上提交并推送新的 commit，这些 commit **会自动同步到已开启的 PR 中**。这意味着：

    - 您**无需关闭旧 PR 再重新创建**，后续所有推送都会自动出现在同一个 PR 中
    - 这一机制在 Review 阶段尤其有用——您只需根据反馈意见修改代码并推送，PR 就会自动更新

    但请注意，如果您不希望某次修改出现在当前 PR 中（例如误提交了无关更改），请在**新分支**上操作，而非继续使用当前 PR 的分支。
